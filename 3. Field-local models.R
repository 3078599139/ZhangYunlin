...
# Use the SAME final field split as Bridge calibration:
#   - training/calibration set: n = 36
#   - validation set:   n = 84
...
if (nrow(train) != 36) {
  stop(
    paste0(
      "Field-local training set has ", nrow(train),
      " samples; expected 36."
    )
  )
}
...
if (nrow(test) != 84) {
  stop(
    paste0(
      "Field-local validation set has ", nrow(test),
      " samples; expected 84."
    )
  )
}
...
feature_vars_all <- setdiff(names(train), response_var)

cat("Field-local training samples :", nrow(train), "\n")
cat("Common validation samples    :", nrow(test), "\n")
...
mlr_metrics <- bind_rows(
  cbind(
    Model = "Field-local MLR",
    Dataset = "Train (n=36)",
    calc_metrics(train$y, mlr_pred_train)
  ),
  cbind(
    Model = "Field-local MLR",
    Dataset = "Validation (n=84)",
...
pdf(
  file.path(save_dir, "FieldLocal_MLR_residual_diagnostics.pdf"),
  width = 8,
  height = 8
)
par(mfrow = c(2, 2))
...
# 6. RF model
# ----------------------------
# mtry is optimized by 5-fold CV using only the 36 field training samples.
# ntree is fixed explicitly at 500.
...
p_oob <- ggplot(
  ...
  labs(
    title = "Field-local RF: OOB error by number of trees",
    x = "Number of trees",
    y = "OOB RMSE"
  ) +
  theme_bw()
...
ggsave(
  file.path(save_dir, "FieldLocal_RF_OOB_error_curve.pdf"),
  p_oob,
  width = 6,
  height = 4.5,
  dpi = 300
...
write.csv(
  all_metrics,
  file.path(save_dir, "Field_local_metrics_36_84.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(save_dir, "Field_local_MLR_diagnostics.csv"),
  row.names = FALSE
)

write.csv(
  rf_param_df,
  file.path(save_dir, "Field_local_RF_best_params.csv"),
  row.names = FALSE
)

write.csv(
  rf_cv_results,
  file.path(save_dir, "Field_local_RF_CV_tuning_results.csv"),
  row.names = FALSE
)

write.csv(
  rf_oob_df,
  file.path(save_dir, "Field_local_RF_OOB_error.csv"),
  row.names = FALSE
)

write.csv(
  rf_importance,
  file.path(save_dir, "Field_local_RF_variable_importance.csv"),
  row.names = FALSE
)
...
write.csv(
  pred_df,
  file.path(save_dir, "Field_local_predictions_36_84.csv"),
  row.names = FALSE
)

saveRDS(
  mlr_model,
  file.path(save_dir, "FieldLocal_MLR_model.rds")
)

saveRDS(
  rf_tune,
  file.path(save_dir, "FieldLocal_RF_model.rds")
)
...
save_jsd_density_plot(
  ...
  mlr_pred_test,
  calc_metrics(test$y, mlr_pred_test),
  "Field-local: MLR (36 train / 84 validation)",
  file.path(save_dir, "FieldLocal_MLR_validation_JSD_density.pdf")
)
...
save_jsd_density_plot(
  ...
  rf_pred_test,
  calc_metrics(test$y, rf_pred_test),
  "Field-local: RF (36 train / 84 validation)",
  file.path(save_dir, "FieldLocal_RF_validation_JSD_density.pdf")
)

print(all_metrics)
cat("\nField-local RF selected mtry =", rf_tune$bestTune$mtry, "\n")
cat("Field-local RF fixed ntree   =", RF_NTREE, "\n")
